...
class Ticket {
  ...
  public static function getById(int $id): ?array {
    ...
    $stmt = $pdo->prepare("
      SELECT t.*,
             u.username  AS username,              -- changed alias for view compatibility
  ...
  public static function store(int $userId, array $data): int {
    ...
    // Resolve requester from unified input (email or username)
    ...
    // include requester_user_id + requester_email in INSERT (you were not using them)
    $stmt = $pdo->prepare("
      INSERT INTO tickets
        (user_id, assigned_to, department_id, title, description, requester_user_id, requester_email, priority)
      VALUES
        (:uid,    :a,         :d,            :t,    :desc,       :ruid,             :remail,          :p)
    ");
    $stmt->execute([
      ':uid'   => $userId ?: null,
      ':a'     => $assigned,
  ...
  public static function searchPaginated(string $q, int $page, int $perPage, array $filters = [], array $currentUser = []): array {
    ...
    // Basic search (LIKE; you can switch to FT later)
    if ($q !== '') {
    ...
    // Permissions (simple version you had)
    ...
    $stmt = $pdo->prepare("SELECT t.*,
               u.username  AS username,              -- changed alias
               au.username AS assigned_username,
               d.name      AS department_name,
               ru.username AS requester_username,
               ru.email    AS requester_user_email
  ...
  private static function resolveRequester(string $input): array {
    ...
    // Unknown username -> nothing to store
